...
export default function App(){
  ...
  const [leadsMode,setLeadsMode]=useState("board");const [showForm,setShowForm]=useState(false);const [showNotifs,setShowNotifs]=useState(false);const [selectedLead,setSelectedLead]=useState(null);const [search,setSearch]=useState("");const [serviceFilter,setServiceFilter]=useState("");const [statusFilter,setStatusFilter]=useState("");const [sourceFilter,setSourceFilter]=useState("");const [taskFilter,setTaskFilter]=useState(false);const [dragId,setDragId]=useState(null);const [sortCol,setSortCol]=useState("created_at");const [sortDir,setSortDir]=useState("desc");const toast=useToast();const [notifRefresh,setNotifRefresh]=useState(0);const notifs=useNotifications(leads,tasks,interactions,notifRefresh);
